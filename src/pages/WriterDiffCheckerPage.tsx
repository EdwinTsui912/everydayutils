...
function normalizeText(text: string, filters: NoiseFilters): string {
    let t = text;
...
function tokenizeWordsWithSpaces(text: string): string[] {
    if (!text) return [];
    // Keep spaces as separate tokens so we can maintain spacing.
    return text.match(/\S+|\s+/g) || [];
}

// Simple LCS-based word diff for writer mode.
    ...
    if (n > 5000 || m > 5000) {
        ...
        for (let i = 0; i < max; i++) {
            ...
            if (L === R) out.push({ op: 'equal', text: L });
                ...
                out.push({ op: 'delete', text: L });
                out.push({ op: 'insert', text: R });
            }
        }
        return out;
    }
...
export default function WriterDiffCheckerPage() {
    ...
    const { tokens, stats, summarySentence } = useMemo(() => {
        const empty = {
            tokens: [] as DiffToken[],
            stats: { added: 0, deleted: 0, equal: 0, similarity: 100 },
            summarySentence: '',
        };
        ...
        let equal = 0;
        tokens.forEach((t) => {
            if (t.op === 'insert') added++;
            else if (t.op === 'delete') deleted++;
            else equal++;
        });
        const total = added + deleted + equal || 1;
        ...
        const delta = wcDraft === 0 ? 0 : Math.round(((wcEdited - wcDraft) / Math.max(wcDraft, 1)) * 100);
        const longer =
            delta > 5
                ? `about ${delta}% longer`
                : delta < -5
        ...
        const summarySentence = `This version is ${longer}. ${added} insertion(s), ${deleted} deletion(s). Similarity ~${similarity}%.`;

        return { tokens, stats: { added, deleted, equal, similarity }, summarySentence };
    ...
    return (
        <div className={`max-w-4xl mx-auto px-4 sm:px-6 py-8 animate-fade-in ${focusView ? 'relative' : ''}`}>
            {/* Hero */}
            <div
                className={`mb-8 text-center transition-opacity ${focusView ? 'opacity-40 pointer-events-none' : ''}`}
            >
            ...
            <div
                className={`card p-6 space-y-6 transition-all ${focusView ? 'ring-2 ring-brand-500/30 shadow-xl' : ''
                    }`}
            >
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <button
                        ...
                        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs border transition-colors shrink-0 ${focusView
                ...
                {!tooLarge && (draft || edited) && (
                    <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-gray-500 dark:text-gray-400">
                            Words added: {stats.added} · Words removed: {stats.deleted} · Similarity: {stats.similarity}%
                        </p>
                    </div>
                )}
                ...
                {!tooLarge && tokens.length > 0 && (
                    <div
                        className={`mt-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 p-4 text-sm leading-relaxed ${focusView ? 'ring-1 ring-brand-500/40' : ''
                            }`}
                    >
                ...
                <div className="mt-4 border-t border-gray-200 dark:border-gray-800 pt-3 flex flex-wrap items-center justify-between gap-3 text-[11px] text-gray-500 dark:text-gray-400">
                    ...
                    <span className="inline-flex items-center gap-1">
                        <FileText size={11} />
                        <Link to="/blog/breathing-timer-focus-stress-sleep" className="text-brand-500 hover:underline">
